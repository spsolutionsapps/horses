...
function SmoothMenuScroll () {
    // Delegación: menú principal, sticky y móvil comparten enlaces; getElementById evita fallos de selector (#the-series, etc.)
    $(document).off('click.hhSmooth', '.scrollToLink > a[href^="#"]').on('click.hhSmooth', '.scrollToLink > a[href^="#"]', function (event) {
        ...
        var headerOff = Math.ceil($('.sticky-header').outerHeight() || 0) || 90;
        if (headerOff < 72) {
            headerOff = 90;
        }
        var scrollPos = Math.round($(el).offset().top - headerOff);
        $('html, body').stop().animate({
            scrollTop: scrollPos
        }, 1200, 'easeInOutExpo');
        $('.scrollToLink').removeClass('current');
        $(this).closest('.scrollToLink').addClass('current');
        $('body').removeClass('mobile-menu-visible');
    });
}
        ...
        $('.mainmenu.one-page-scroll-menu .scrollToLink').find('a').each(function (){
            ...
            $(sections).each(function() {
                ...
                if ($(this).offset().top <= windscroll + 100) {
                    ...
                    // adding current class to related navigation (match exact href; evita fallos con #the-series, #partners)
                    $('.mainmenu').find('a[href="' + sections + '"]').parent().addClass('current');
                }
            });
        });
    } else {
...
// In Production 2026 — horses background parallax (vertical background)
if($('.production-2026-parallax').length){
    $('.production-2026-parallax').paroller({
        factor: 0.32,
        factorLg: 0.28,
        factorMd: 0.22,
        factorSm: 0.14,
...
// Dom Ready Function (use .ready — .on('ready') está deprecado y puede no ejecutarse)
jQuery(document).ready(function () {
	(function ($) {
        ...
        $('#contactModal').on('hidden.bs.modal', function () {
            $('#contact-modal-feedback').addClass('d-none').text('');
        });
        $(document).on('click', 'a[href="contact.html"]', function (e) {
            e.preventDefault();
        ...
        function hhInitProductionCarousel($el, startIndex) {
            ...
            var idx = typeof startIndex === 'number' && !isNaN(startIndex) ? startIndex : 0;
            setTimeout(function () {
        ...
        $('#productionModalKing').on('show.bs.modal', function (e) {
            var $t = $(e.relatedTarget);
            productionKingSlide = ($t.length && $t.is('a.production-card__zoom--king')) ? (parseInt($t.data('slide'), 10) || 0) : 0;
        });
        $('#productionModalKing').on('shown.bs.modal', function () {
            hhInitProductionCarousel($('#owlProductionKing'), productionKingSlide);
        });
        $('#productionModalAz').on('show.bs.modal', function (e) {
            var $t = $(e.relatedTarget);
            productionAzSlide = ($t.length && $t.is('a.production-card__zoom--az')) ? (parseInt($t.data('slide'), 10) || 0) : 0;
        });
        $('#productionModalAz').on('shown.bs.modal', function () {
            hhInitProductionCarousel($('#owlProductionAz'), productionAzSlide);
        });
